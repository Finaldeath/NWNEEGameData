...
void cs_StartCutscene(string sCSScript, string sEndScript="", float fDelay=0.0, int bAbortable=FALSE, int bRemoveAOEs=TRUE, int nCutNumber=1, object oArea=OBJECT_SELF)
{
    ...
    if(GetLocalInt(oArea, "cs_bCutsceneStarted"))
    {
    ...
    SetLocalString(oArea, "cs_sEndScript", sEndScript);
    ...
    vTemp.y = 1.0;
    vTemp.z = 1.0;
    object oEffectObject = CreateObject(OBJECT_TYPE_PLACEABLE, "plc_invisobj", Location(oArea, vTemp, 0.0), FALSE, "cs_EffectObject");
    SetLocalObject(oArea, "cs_oEffectObject", oEffectObject);

    // Initialize actor array index (actor is a broad term meaning any object involved in the cutscene)
    SetLocalInt(oArea, "cs_nActorNum", 0);

    // Place all players in the area into cutscene mode.
    // Put them into an array. First PC is number 0.
    // If there are 4 PC's, cs_nNumPC will be 4, numbered 0 to 3.
    int nNumPC = 0;
    object oPC = GetFirstPC();
    SetLocalInt(oArea, "cs_nCameraMode", CAMERA_MODE_TOP_DOWN);
    ...
    SetLocalInt(oArea, "cs_nNumPC", nNumPC);
    SetLocalObject(oArea, "cs_oVisiblePC", OBJECT_INVALID);


    string sCutNumber = IntToString(nCutNumber);
    SetLocalInt(oArea, "cs_nCutNumber", nCutNumber);
    SetLocalInt(oArea, "cs_bAbortable", FALSE);
    DelayCommand(fDelay+0.1, SetLocalInt(oArea, "cs_bAbortable", bAbortable));
    //DelayCommand(fDelay, SetLocalInt(oArea, "cs_bCutsceneStarted", TRUE));
    SetLocalInt(oArea, "cs_bCutsceneStarted", TRUE);
    SetLocalInt(oArea, "cs_bCutsceneAborted"+sCutNumber, FALSE);
    //DelayCommand(fDelay, SetLocalInt(oArea, "cs_bCutsceneStarted"+sCutNumber, TRUE));
    SetLocalInt(oArea, "cs_bCutsceneStarted"+sCutNumber, TRUE);
    SetLocalInt(oArea, "cs_bCutsceneEnded"+sCutNumber, FALSE);

    // Set delay type variables
    int nGlobalDelayType = GetLocalInt(GetModule(), "cs_g_DelayType");
    ...
    SetLocalFloat(oArea, "cs_fTotalDelay", 0.0);

    // set some default camera values, just in case
    SetLocalLocation(oArea, "cs_lCameraLocation", GetLocation(GetObjectArray(oArea, "cs_oPC", 0)));
    SetLocalFloat(oArea, "cs_fCameraDirection", 0.0);
    SetLocalFloat(oArea, "cs_fCameraDistance", 10.0);
    SetLocalFloat(oArea, "cs_fCameraPitch", 30.0);
    SetLocalFloat(oArea, "cs_fCameraHeight", 0.0);
    ...
    if(sCSScript=="") DelayCommand(fDelay+0.05, ExecuteScript("cs_"+GetTag(oArea), oArea));
...
void cs_EndCutscene(float fDelay, int bStopFade=TRUE, int bFadeFromBlack=FALSE, float fSpeed=FADE_SPEED_MEDIUM, int nCutNumber=-1, object oArea=OBJECT_SELF)
{
    ...
    if(!GetLocalInt(oArea, "cs_bCutsceneStarted")) return;
...
void cs_AbortCutscene(int nCutNumber=1, object oArea=OBJECT_SELF)
{
    ...
    SetLocalInt(oArea, "cs_bCutsceneAborted"+IntToString(nCutNumber), TRUE);
...
void cs_ModuleOnCutsceneAbort(object oLastPCToCancelCutscene)
{
    ...
    int bAbortable = GetLocalInt(oArea, "cs_bAbortable");
    int bCutsceneStarted = GetLocalInt(oArea, "cs_bCutsceneStarted");
...
void cs_SetOption(int nOptionToSet, int bValue, object oArea=OBJECT_SELF)
{
    if(bValue == -1)
    {
        DeleteLocalInt(oArea, "cs_option"+IntToString(nOptionToSet));
        return;
    }
    ...
    SetLocalInt(oArea, "cs_option"+IntToString(nOptionToSet), bValue);
}
...
void cs_SetOptionDefault(int nOptionToSet, int bNewDefaultValue)
{
    ...
    {
        DeleteLocalInt(GetModule(), "cs_option"+IntToString(nOptionToSet));
        return;
    }
    ...
    SetLocalInt(GetModule(), "cs_option"+IntToString(nOptionToSet), bNewDefaultValue);
}
...
PC options:
    CS_OPTION_ALL_PCS:    *** NOT CURRENTLY IMPLEMENTED ***
      Default: FALSE
        If TRUE, all PC's in the entire module are jumped into the cutscene.
        If FALSE, only PC's in the area are placed into the cutscene.
