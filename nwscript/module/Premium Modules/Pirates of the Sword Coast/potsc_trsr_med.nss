...
//:://////////////////////////////////////////////
/*
    Create a Medium level treasure on the chest.
...
const int iMaxGold = 200;

void main()
{
    object oChest = OBJECT_SELF;
    int iGold = Random(iMaxGold) + 1;
    int iDoOnce = GetLocalInt(oChest, "potsc_trsr_med");

    // if this is the first time create some random treasure
    if (iDoOnce == FALSE)
    {
        SetLocalInt(oChest, "potsc_trsr_med", TRUE);
        CreateItemOnObject("potsc_goldpiece", oChest, iGold);

        CreateBootyOnObject(oChest, BOOTY_GENERIC);
        CreateBootyOnObject(oChest, BOOTY_GENERIC);
    }
}
