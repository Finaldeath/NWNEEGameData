...
string GetUniqueItem()
{
    ...
    while (iDone == FALSE)
    {
        iUnique = GetLocalInt(oModule, S_UNIQRESREF + IntToString(iRandom));

        if (iUnique != TRUE)
        {
            sBootyResRef = GetLocalString(oModule, S_UNIQRESREF + IntToString(iRandom));
            SetLocalInt(oModule, S_UNIQRESREF + IntToString(iRandom), TRUE);
            iDone = TRUE;
        }

        //if you get to the bottom of the list try again once.
        if (iRandom <= 0)
        {
            if (iRestart == FALSE)
            {
...
object CreateBootyOnObject(object oTarget, int iFlag)
{
    ...
    if (iFlag == BOOTY_UNIQUE)
    {
        ...
        if (sBootyResRef == S_UNIQFAILED)
        {
            for (iLoop = 0; iLoop <= 4; iLoop++)
            {
                oCreate = CreateItemOnObject(GetGenericItem(), oTarget);
            }
        }
        else
        {
    ...
    {
        SetItemStackSize(oCreate, Random(40) + 11);
    }
