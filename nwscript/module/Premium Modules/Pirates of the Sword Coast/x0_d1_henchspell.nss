...
void DoBuff(int nBuffType, object oTarget)
{
    ...
    if(nBuffType == 4) // PotSC: Negative energy spells for undead players
    {
        HandleSpell(SPELL_INFLICT_CRITICAL_WOUNDS, 1);
        HandleSpell(SPELL_NEGATIVE_ENERGY_RAY, 1);
        HandleSpell(SPELL_INFLICT_SERIOUS_WOUNDS, 1);
        HandleSpell(SPELL_INFLICT_MODERATE_WOUNDS, 1);
        HandleSpell(SPELL_CIRCLE_OF_DOOM, 1);
        HandleSpell(SPELL_NEGATIVE_ENERGY_BURST, 1);
        HandleSpell(SPELL_INFLICT_LIGHT_WOUNDS, 1);
        HandleSpell(SPELL_INFLICT_MINOR_WOUNDS, 1);
        HandleSpell(SPELL_HARM);
        HandleSpell(SPELL_STONE_BONES);
    }
