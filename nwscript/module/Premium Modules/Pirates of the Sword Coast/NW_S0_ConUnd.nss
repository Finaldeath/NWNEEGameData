...
// End of Spell Cast Hook
    ...
    effect eWis = EffectAbilityDecrease(ABILITY_WISDOM, d4(1));
    effect eDur = EffectVisualEffect(VFX_DUR_CESSATE_NEGATIVE);
    effect eVis = EffectVisualEffect(VFX_IMP_DOMINATE_S);

    effect eLink = EffectLinkEffects(eMind, eControl);
    effect eLink2 = EffectLinkEffects(eMind, eWis);
        ...
        && (GetHitDice(oTarget) <= nHD))
    {
       ...
       if (!MyResistSpell(OBJECT_SELF, oTarget))
       {
            ...
            if (!/*Will Save*/ MySavingThrow(SAVING_THROW_WILL, oTarget, GetSpellSaveDC(), SAVING_THROW_TYPE_NONE, OBJECT_SELF, 1.0))
            {
                ...
                {
                    DelayCommand(1.0, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink2, oTarget, HoursToSeconds((nDuration/2))));
                }
                else
                {
