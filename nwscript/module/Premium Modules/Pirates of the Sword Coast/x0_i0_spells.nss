        ...
        effect eLink = EffectLinkEffects(eDur, ePetrify);
            ...
            if (!MySavingThrow(SAVING_THROW_FORT, oTarget, nSaveDC))
            {
                ...
                if (bIsPC == TRUE)
                {
                    if (bShowPopup == TRUE)
                    {
                        string sDeath = "There is no penalty for respawning. You will be "+
                                        "returned to a safe point in the module.";

                        // * under hardcore rules or higher, this is an instant death
                        ApplyEffectToObject(DURATION_TYPE_PERMANENT, eLink, oTarget);
                        DelayCommand(2.75, PopUpDeathGUIPanel(oTarget, TRUE , TRUE, 0, sDeath));
...
int spellsIsMindless(object oCreature)
{
    ...
    if (GetLocalInt(oCreature, "IOP_CUST_UNDEAD"))
