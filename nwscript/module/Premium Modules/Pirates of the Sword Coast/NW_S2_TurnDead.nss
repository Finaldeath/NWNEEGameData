...
const string S_UNDEAD_CASTER = "On second thought, you'd better wait until you're no longer undead yourself.";
...
void main()
{
    ...
    object oTarget = GetFirstObjectInShape(SHAPE_SPHERE, 20.0f, lSelf, TRUE, OBJECT_TYPE_CREATURE);
    ...
    while(GetIsObjectValid(oTarget) && nHDCount < nTurnHD && GetDistanceToObject(oTarget) <= 20.0)
    {
            ...
            if(nHD <= nTurnLevel && nHD <= (nTurnHD - nHDCount))
            {
                ...
                if( bValid == TRUE)
                {
                    ...
                    if (GetIsPC(oTarget) == TRUE)
                    {
                        ...
                        {
                            AssignCommand(oTarget, ActionMoveAwayFromLocation(lSelf, TRUE));
                        }
                        else
                        {
        ...
        nCnt++;
        oTarget = GetNextObjectInShape(SHAPE_SPHERE, 20.0f, lSelf, TRUE, OBJECT_TYPE_CREATURE);
    }
}
