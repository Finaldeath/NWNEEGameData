...
void main()
{
    ...
    //sets animations for Veiti so that she tends the workshop fire
    {
    if (IsInConversation(OBJECT_SELF) == FALSE && GetIsInCombat() == FALSE &&
        !GetLocalInt(GetModule(),"veiti_ambient")==1)
        {
        ActionDoCommand(SetFacingPoint(GetPosition(GetNearestObjectByTag("WP_Furnace"))));
