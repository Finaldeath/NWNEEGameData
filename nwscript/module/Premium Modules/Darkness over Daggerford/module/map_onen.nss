...
void main()
{
    ...
    PrintString("Player entered world map");
    ...
    if (GetIsPC(oEnterer))
    {
        ...
        while(oHostile != OBJECT_INVALID)
        {
            oHostile = GetNearestCreature(CREATURE_TYPE_REPUTATION,
                                          ...
                                          oEnterer,
                                          i);
            PrintString("On map enter - Nearest enemy = " + GetTag(oHostile));
            if(GetIsObjectValid(oHostile) &&
               GetAssociateType(oHostile) == ASSOCIATE_TYPE_NONE)
            {
                PrintString("On map enter - Destroying hostile");
        ...
        //AssignCommand(oEnterer, ActionDoCommand(BlackScreen(oEnterer)));
    ...
    {
        // this must be a associate creature
        PrintString("World map entered by " + GetTag(oEnterer));
