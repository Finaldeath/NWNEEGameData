...
void SetRandomPossessions(object oNPC, int nLootable=FALSE)
{
    ...
    CreateItemOnObject("nw_it_gold001", oNPC, d4(1));
}
...
int ActionGoHome()
{
    if (GetDistanceBetweenLocations(GetLocation(OBJECT_SELF), GetLocalLocation(OBJECT_SELF, "HF_SPAWN_LOC")) > 5.0)
    {
        SetIsWalking(GetLocalLocation(OBJECT_SELF, "HF_SPAWN_LOC"), FALSE);
        return (TRUE);
    }
    return(FALSE);
}
        ...
        if (GetIsObjectValid(oMark))
        {
            if (!GetIsUsed(oMark))
            {
                ...
                else
                {
                    ...
                    if (GetJob(oMark) == "THUG" || GetJob(oMark) == "ROGUE")
                    {
                        return(FALSE);
                    }
                    ...
                    ActionDoCommand(AssignCommand(oMark, ActionSpeakString("Hey!")));
...
int ActionPerform()
{
    if (GetJob() == "BARD")
    {
        ...
        else if (nState <= 3)
        {
            ...
            if (GetLocalString(OBJECT_SELF, "HF_ITEM") == "FLUTE")
            {
