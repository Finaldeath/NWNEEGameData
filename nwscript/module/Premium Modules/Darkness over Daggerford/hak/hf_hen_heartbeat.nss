...
void main()
{
    ...
    // if dead and no master around, then go home
    // (code removed during play-testing)
    /*
    if (HenchmanIsDead(OBJECT_SELF))
    {
        if (GetMaster(OBJECT_SELF) == OBJECT_INVALID)
        {
            object oLastMaster = GetLocalObject(OBJECT_SELF, "HF_HENCHMAN_LAST_MASTER");
            if (GetIsObjectValid(oLastMaster) && !GetIsDead(oLastMaster))
            {
                if (GetArea(oLastMaster) != GetArea(OBJECT_SELF))
                {
                    HenchmanRaise(OBJECT_SELF, OBJECT_INVALID);
