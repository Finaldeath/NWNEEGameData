...
/*
  ...
  OnDialogue event handler for Gofrey as henchman
  Can't be spelled from party via radial menu while
  nMainPlot < 3
...
//:: Based on default OnDialogue script
//:://////////////////////////////////////////////////
    ...
    else
    {
        ...
        if (GetIsObjectValid(oMaster))
        {
            ...
            if (GetIsObjectValid(oShouter) && oMaster == oShouter && AbleToTalk(OBJECT_SELF)) {
                SetCommandable(TRUE);
                if (nMatch == ASSOCIATE_COMMAND_LEAVEPARTY && GetLocalInt(oMaster, "nMainPlot") < 3 )
                {
                  return;
                }
                else
