...
// 5 = prayed to release the spirit from the blade
...
//  ScoreAlign(oPC,Max,LG,NG,CG,LN,TN,CN,LE,NE,CE);            //Who gets alignment points for this - set in script s9 or s6
    ...
    PartyGotEntry(sName,oPC,300,"XP Gain - poltergeist vanquished");  //xp for defeat of poltergeist
    if (GetLocalInt(oPC,"nPoltergeist")>0) //the following updates only if Magnus spoken to
    {
        AddJournalQuestEntry("Poltergeist",21,oPC,TRUE);  //Update the journal (spoke to Magnus)
