...
// Alignment consequences: This is not a lawful outcome, but is otherwise non-aligned as the player
// does not know for sure whether the document is real. Slight good penalty for not checking.
...
//    if(GetAlignmentLawChaos(oPC)==ALIGNMENT_LAWFUL) AligXP("AligXP2a", oPC, "CHAOTIC", 0, 2); //Apply to specific alignment
    AligXP("AligXPImRev3", oPC, "CHAOTIC", 100, 1);        //Shift/reward any alignment
...
//    FloatingTextStringOnCreature("XP Gained - use of skill",GetPCSpeaker());
    ...
    AddJournalQuestEntry("Revenge",25,oPC,TRUE);                      //Update the journal
    ScoreAlign(oPC,5,0,4,4,1,5,5,2,5,5);            //Who gets alignment points for this
    }
