   ...
   { case X2_ITEM_EVENT_EQUIP:  //Equip event for Deity Specific powers.
         {
            ...
            if (GetDeity(oPC)=="Lathander" || GetDeity(oPC)=="Tyr" || GetDeity(oPC)=="Tempus"  || GetDeity(oPC)=="Helm") //check for a matching deity on the PC
            {
            //Deity Specific items generally give 2 or more powers, a 2/day power, and a temporary effect.
            itemproperty eProp = ItemPropertyCastSpell(IP_CONST_CASTSPELL_LIGHT_1,9);
            itemproperty eProp2 = ItemPropertyACBonusVsRace(IP_CONST_RACIALTYPE_UNDEAD,2);
