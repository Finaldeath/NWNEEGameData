...
void main()
{
    ...
    //must to bail out if it is not the PCs turn to play.
    if(GetLocalInt(GetFirstPC(),"nPlayingDarts") && !GetLocalInt(GetFirstPC(),"nMyDartTurn"))
    {
        return;
    }
    //bail out if in dummy contest
    if(GetLocalInt(GetFirstPC(),"nDummyContestActive"))
    {
        return;
    }
