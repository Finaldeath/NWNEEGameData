...
// they have a deity power on them. Note, although the resref and tag are mis-spelt, the Deity in the script is either
   ...
   { case X2_ITEM_EVENT_EQUIP:  //Equip event for Deity Specific powers.
         {
            ...
            if (GetDeity(oPC)=="Cyric" || GetDeity(oPC)=="Cyrric") //check for a matching deity on the PC
            {
            //Deity Specific items generally give 2 or more powers, a 2/day power, and a temporary effect.
            itemproperty eProp = ItemPropertySkillBonus(SKILL_MOVE_SILENTLY,7);
            itemproperty eProp2 = ItemPropertySkillBonus(SKILL_HIDE,7);
