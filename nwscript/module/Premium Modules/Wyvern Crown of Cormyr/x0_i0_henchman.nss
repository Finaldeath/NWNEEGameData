// Same as basic x0_i0_henchman, except has the classing rules for Wyvern Crown instead of OC/HotU.
// Modified for henchmen levelling - B W-Husey, Ongoing
// Modified March 19th 2006
// Modified 8th April, removed henchman lag
// Modified 9th April, Jonas fixes
// Modified 2nd August - re-implemented GetCanLevelUp function
...
/**** Number of henchmen ****/
int X0_NUMBER_HENCHMEN = 2;

int X2_NUMBER_HENCHMEN = 2; //MODIFIED***************
...
int GetCanLevelUp(object oPC, object oHench = OBJECT_SELF)
{
    if (GetHitDice(oPC) == GetHitDice(oHench))
    return FALSE;
    else
    return TRUE;
}
    ...
    int nMaxLevelInSecondClass=0, int nPackageClass1=PACKAGE_INVALID, int nPackageClass2=PACKAGE_INVALID)
{
    ...
    int nClassToLevelUp;   //added stuff
...
//        if ( nClass2 != CLASS_TYPE_INVALID && (nLevel1 > nLevel2) )
        ...
        if ((nLevel1+nLevel2)<6) //override and force single classing for Jonas
        {
        if (GetTag(oHenchman) == "Jonas")
                {
                    nClassToLevelUp = CLASS_TYPE_FIGHTER;
                    nPackageToUse = nPackageClass1;
                }
        }
        if ((nLevel1+nLevel2)<4) //override and force single classing for Lacinda & Godfroy
        {
            if (GetTag(oHenchman) == "Lacinda")
                {
                    nClassToLevelUp = CLASS_TYPE_SORCERER;
                    nPackageToUse = nPackageClass1;
                }
            else if (GetTag(oHenchman) == "Godfroy")
                {
                    nClassToLevelUp = CLASS_TYPE_FIGHTER;
                    nPackageToUse = nPackageClass1;
                }
        ...
        //**********Debug Line***************
//        SendMessageToPC(GetFirstPC(),"Class after 1st check "+IntToString(nClassToLevelUp));
        ...
        //Gale 19th May 2006: Rules where wrong, fixed
        if (nRule == 1)
        {
            nClassToLevelUp = nClass2;
            nPackageToUse = nPackageClass2;
        ...
        if (nLevel1+nLevel2>8) // Jonas switches to Blackguard
        {
            if (GetTag(oHenchman) == "Jonas")
            {
                    if (GetAlignmentGoodEvil(oHenchman)==ALIGNMENT_EVIL)
                    {
                        nClassToLevelUp = CLASS_TYPE_BLACKGUARD;
                        nPackageToUse = PACKAGE_BLACKGUARD;
                    }
                    else
                    {
                        nClassToLevelUp = CLASS_TYPE_FIGHTER;
        ...
        //**********Debug Line***************
//        SendMessageToPC(GetFirstPC(),"Class after rule check "+IntToString(nClassToLevelUp));

//        if (nClasshold == nClass1) nClassToLevelUp = nClass2;
//        nClasshold == nClassToLevelUp;



        LevelUpHenchman(oHenchman, nClassToLevelUp, TRUE, nPackageToUse);



/*        if (!LevelUpHenchman(oHenchman, nClassToLevelUp, TRUE, nPackageToUse))
        {
...
int AdjustXP2Levels(int nLevel, int nMin=6, int nAdjust=2)
{
...
// *
// Gale - April 19, 2006: Fixing to work with all henchmen (horses were breaking it)
void LevelUpXP1Henchman(object oPC)
{
    ...
    //**********Debug Line***************
//    SendMessageToPC(GetFirstPC(),"XP1 function run ");


    int i = 1;
    object oAssociate;
    for (i=1; GetIsObjectValid(oAssociate = GetAssociate(ASSOCIATE_TYPE_HENCHMAN, oPC, i)); i++)
    {
        oAssociate = GetAssociate(ASSOCIATE_TYPE_HENCHMAN, oPC, i);

        //**********Debug Lines***************
//        SendMessageToPC(GetFirstPC(),"XP1 function returned a henchman");
//        SendMessageToPC(GetFirstPC(),GetName(oAssociate));
        ...
        if ( GetIsObjectValid(oAssociate) )
        {
            ...
            //**********Debug Line***************
//            SendMessageToPC(GetFirstPC(),"XP1 function returned a valid henchman");
                ...
                {
                    nLevel = AdjustXP2Levels(nLevel,6,0);
                    LevelHenchmanUpTo(oAssociate, nLevel, CLASS_TYPE_PALADIN);
                }
                else if (sTag == "jonas")
                {
                    nLevel = AdjustXP2Levels(nLevel,6,0);
//                    LevelHenchmanUpTo(oAssociate, nLevel, CLASS_TYPE_ROGUE, 20,PACKAGE_FIGHTER_PIRATE,PACKAGE_ROGUE_BANDIT);
                    if (GetAlignmentGoodEvil(oAssociate)==ALIGNMENT_EVIL)
                    LevelHenchmanUpTo(oAssociate, nLevel, CLASS_TYPE_BLACKGUARD, 20,PACKAGE_FIGHTER_PIRATE,PACKAGE_BLACKGUARD);
                    else
                    LevelHenchmanUpTo(oAssociate, nLevel, CLASS_TYPE_FIGHTER);

                }
                else if (sTag == "kathryn")
                {
                    nLevel = AdjustXP2Levels(nLevel,6,0);
                    LevelHenchmanUpTo(oAssociate, nLevel, CLASS_TYPE_RANGER,20,PACKAGE_ROGUE_SWASHBUCKLER,PACKAGE_RANGER_STALKER);
                }
                else if (sTag == "lacinda")
                {
                    nLevel = AdjustXP2Levels(nLevel,6,0);
                    LevelHenchmanUpTo(oAssociate, nLevel, CLASS_TYPE_ROGUE, 20,PACKAGE_SORCERER,PACKAGE_ROGUE_SCOUT);
                }
                else if (sTag == "godfroy")
                {
                      //**********Debug Line***************
//                    SendMessageToPC(GetFirstPC(),"Ran class check on Godfroy");

                    nLevel = AdjustXP2Levels(nLevel,6,0);
//                    LevelHenchmanUpTo(oAssociate, nLevel);
//                    LevelHenchmanUpTo(oAssociate, nLevel, CLASS_TYPE_PURPLEDRAGON, 20,PACKAGE_FIGHTER_COMMANDER,PACKAGE_PURPLEDRAGON);
                    LevelHenchmanUpTo(oAssociate, nLevel, CLASS_TYPE_CLERIC, 20,PACKAGE_FIGHTER_COMMANDER,PACKAGE_CLERIC_DIVINE_2NDCLASS);
                }
                else
                // * Aribeth
                if (sTag =="h2_aribeth")                {
                ...
                else if (GetLocalInt(oAssociate, "X2_JUST_A_FOLLOWER") == FALSE)
                {
                    LevelHenchmanUpTo(oAssociate, nLevel);
                }
