...
void main()
{
   ...
   if (GetIsObjectValid(oItem))
   {
     ...
     // run the generic VFX
     int nVfx = GetLocalInt(oItem, "HF_ITEM_ONHIT_VFX");
     if (nVfx > 0)
     {
        if (d100() <= 5)
        {
            ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectVisualEffect(nVfx), oSpellTarget);
        }
     }

     // run the poison effect
     int nPoisonId = GetLocalInt(oItem, "HF_ITEM_ONHIT_POISON");
     if (nPoisonId > 0)
     {
        PoisonOnHit(oSpellOrigin, oItem, oSpellTarget);
     }
