...
void main()
{
    ...
    string sPaperTag = "rag_nspr_i" + IntToString(GetCurrentDay());

    // 2019-08-22 JY
    // Give a copy to all the players.
    ...
    while(GetIsObjectValid(oFactionMember))
    {
        if(HasItem(oFactionMember, sPaperTag) == FALSE)
        {
            ...
            object oNewspaper = CreateItemOnObject("renegaderag", oFactionMember, 1, sPaperTag);

            SetName(oNewspaper, "Renegade Rag: Day " + IntToString(GetCurrentDay()));
    ...
    SpeakString("You got a copy of today's rag.");
}
