...
void main()
{
    ...
    SetLocalInt(GetModule(), "ddf_finished_nightmare_02", TRUE);
    ...
    debug(IntToString(GetIsObjectValid(oPC)) + " " + IntToString(GetIsObjectValid(oLocation)) + " " + IntToString(GetIsObjectValid(oPartyMember)));
        ...
        while(GetIsObjectValid(oPartyMember))
        {
            if(!GetIsDead(oPartyMember))
            {
                ...
                eDrain = SupernaturalEffect(eDrain); // To make it more permanent -- https://app.asana.com/0/842177838074622/1116978695178841
...
void UpdateXPAndDay(object oPC)
{
    ...
    object oPlaceable = GetObjectByTag("NMARE_TWO");
    AssignCommand(oPlaceable, ActionStartConversation(oPC));
}
