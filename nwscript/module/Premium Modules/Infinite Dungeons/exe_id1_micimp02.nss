...
{
    DebugMessage("");
    DebugMessage("Importing wall decor.");
    ...
    int nNum = GetLocalInt(oImport, "nListDecorWallNum");
    ...
    {
        sResRef = GetLocalString(oImport, "sListDecorWall" + IntToString(nImport) + "ResRef");
    ...
    } else
    {
        ...
        {
            object oMicroCPU = CreateObject(OBJECT_TYPE_PLACEABLE, "cpu", GetLocation(GetWaypointByTag("CPU")), FALSE, "MicroCPU74");
            SetLocalObject(oMicroCPU, "oParent", oParent);
            SetLocalObject(oMicroCPU, "oImport", oImport);
            SetLocalInt(oMicroCPU, "nImport", nImport);
            DelayCommand(0.01, ExecuteScript("exe_id1_micimp02", oMicroCPU));
        }
    }
