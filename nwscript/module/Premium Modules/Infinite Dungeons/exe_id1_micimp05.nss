...
{
    DebugMessage("");
    DebugMessage("Importing containers.");
    ...
    int nNum = GetLocalInt(oImport, "nListContainerNum");
    ...
    {
        sResRef = GetLocalString(oImport, "sListContainer" + IntToString(nImport) + "ResRef");
    ...
    } else
    {
        ...
        {
            object oMicroCPU = CreateObject(OBJECT_TYPE_PLACEABLE, "cpu", GetLocation(GetWaypointByTag("CPU")), FALSE, "MicroCPU77");
            SetLocalObject(oMicroCPU, "oParent", oParent);
            SetLocalObject(oMicroCPU, "oImport", oImport);
            SetLocalInt(oMicroCPU, "nImport", nImport);
            DelayCommand(0.01, ExecuteScript("exe_id1_micimp05", oMicroCPU));
        }
    }
