...
void main()
{
    ...
    switch (nListType)
    {
        ...
        case 128:
            ...
            sString += "Encounter difficulty is ";
            if (nTemp < 0)
            {
                sString += "-";
            } else
        ...
        case 131:
            ...
            sString += GetLocalString(oDungeon, "sCreature" + IntToString(nTemp) + "NamePlural");
            sString += ".";
            break;
        case 132:
            sString += "Encounter creatures are always from ";
            nTemp = GetModuleFlagValue(ID1_FLAG_ENCOUNTER_TYPE);
            sString += GetLocalString(oDungeon, "sTheme" + IntToString(nTemp) + "Name");
            sString += ".";
            break;
        case 133:
        ...
        case 134:
            sString += "The final boss area for this dungeon is ";
            nTemp = GetModuleFlagValue(ID1_FLAG_BOSS_AREA_SELECTION);
            nTemp = GetLocalInt(GetModule(), "nBossThemeSetArea" + IntToString(nTemp));
            oTemp = GetLocalObject(GetModule(), "oAreaIdentifier" + IntToString(nTemp));
            sTemp = GetName(oTemp);
            sString += sTemp;
            sTemp = GetTag(oTemp);
