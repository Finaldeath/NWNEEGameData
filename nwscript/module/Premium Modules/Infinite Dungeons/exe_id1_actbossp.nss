...
void main()
{
    ...
    string sWaypoint = "Area" + IntToString(nAreaNum) + "WaypointBoss";
    string sGuardWaypoint = "Area" + IntToString(nAreaNum) + "WaypointBossGuard";

    // get number of encounter waypoints
    int nWaypointNum = GetLocalInt(oDungeon, "n" + sWaypoint);
    int nGuardWaypointNum = GetLocalInt(oDungeon, "n" + sGuardWaypoint);

    DebugMessage("There are " + IntToString(nWaypointNum) + " boss and " + IntToString(nGuardWaypointNum) + " guard points.");
    ...
    DebugMessage("Spawning " + IntToString(nWaypointNum) + " boss points.");
    ...
    while (nWaypoint <= nWaypointNum)
    {
        ...
        if (GetLocalInt(oDungeon, "b" + sLevel + "Used") == TRUE)
        {
            DebugMessage("Waypoint " + IntToString(nWaypoint) + " is used.");

            oWaypoint = GetLocalObject(oArea, "oWaypointBoss" + IntToString(nWaypoint));
            ...
            while (nCreature <= nCreatureNum)
            {
                ...
                if (bDead == FALSE)
                {
                    ...
                    // if nothing, spawn a penguin
                    if (sResRef == "")
                    {
                        sResRef = "id1_penguin";
                    }

                    DebugMessage("Spawning MiniBoss " + IntToString(nCreature) + " with resref " + sResRef);

                    oCreature = CreateObject(OBJECT_TYPE_CREATURE, sResRef, GetLocation(oWaypoint));

                    // April 4, 2006 GM: Flag mini bosses as mini bosses for the
                    // score keeping/tracking system.
                    SetLocalInt(oCreature, "bMiniBoss", TRUE);
    ...
    DebugMessage("Spawning " + IntToString(nGuardWaypointNum) + " boss guard points.");

    nWaypoint = 1;
    while (nWaypoint <= nGuardWaypointNum)
    {
        sLevel = sGuardWaypoint + IntToString(nWaypoint) + "Level" + IntToString(nCurrentLevel);
        ...
        if (GetLocalInt(oDungeon, "b" + sLevel + "Used") == TRUE)
        {
            ...
            oWaypoint = GetLocalObject(oArea, "oWaypointBossGuard" + IntToString(nWaypoint));
