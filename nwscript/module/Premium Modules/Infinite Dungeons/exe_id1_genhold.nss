...
void main()
{
    ...
    DebugMessage("");
    DebugMessage("Holding generation with " + IntToString(nMicroNum) + " active and " + IntToString(nMicroDead) + " dead microprocesses.");

    // if waiting on micro processes
    if (nMicroDead < nMicroNum)
    {
        DelayCommand(0.01, ExecuteScript("exe_id1_genhold", GetNextCPU()));
    } else // all microprocesses done
    {
        ...
        DebugMessage("Returning to exe_id1_levelgen.");
