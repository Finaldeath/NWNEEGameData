...
// Set TRUE to have log entries time stamped.
const int TIME_STAMP = FALSE;
...
void DebugMessage(string sMsg)
{
    ...
    if (GetLocalInt(GetModule(), "bDebugMode") == TRUE)
    {

        // All player's within 10 meters of the object running the script will
        // recieve the debug message.
        if (GetLocalInt(GetModule(), "bDebugModeP") == TRUE)
        {
            object oThis = OBJECT_SELF;

            location lTarget = GetLocation(oThis);

            object oTarget = GetFirstObjectInShape(SHAPE_SPHERE, 20.0, lTarget);
            ...
            while (GetIsObjectValid(oTarget))
            {
                if (GetIsPC(oTarget))
                {
                    DelayCommand(fDelay, SendMessageToPC(oTarget, sMsg));
                    fDelay += 0.1;
                }

                oTarget = GetNextObjectInShape(SHAPE_SPHERE, 20.0, lTarget);
            }
        }

        // Write a log entry to the clientlog file.
        if (TRUE)
        {
            if (GetLocalInt(GetModule(), "bDebugModeT") == TRUE)
            {
...
object DB_CreateObject(int nObjectType, string sTemplate, location lLocation, int bUseAppearAnimation = FALSE, string sNewTag = "")
{
    ...
    if (DEBUG_MODE)
    {
        ...
        {
            object oPC = GetFirstPC();
            string sDebug = "OBJECT_INVALID::DB_CreateObject";
