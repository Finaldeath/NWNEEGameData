...
void main()
{
    ...
    int nNum = GetLocalInt(oImport, "nListCreatureNum");
    ...
    {
        sResRef = GetLocalString(oImport, "sListCreature" + IntToString(nImport) + "ResRef");
        oObject = CreateObject(OBJECT_TYPE_CREATURE, sResRef, lLocation);
    ...
    } else
    {
        ...
        {
            object oMicroCPU = CreateObject(OBJECT_TYPE_PLACEABLE, "cpu", GetLocation(GetWaypointByTag("CPU")), FALSE, "MicroCPU80");
            SetLocalObject(oMicroCPU, "oParent", oParent);
            SetLocalObject(oMicroCPU, "oImport", oImport);
            SetLocalInt(oMicroCPU, "nImport", nImport);
            DelayCommand(0.01, ExecuteScript("exe_id1_micimp08", oMicroCPU));
        }
    }
