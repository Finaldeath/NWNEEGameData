...
/*
  ...
  Many functions removed to separate libraries:
    ...
    ***********************************************'
    ...
    June 30, 2005 (Jason Lowry): Modified DetermineCombatRound to turn off Detect Mode.
    ********************************************
...
void DetermineCombatRound(object oIntruder = OBJECT_INVALID, int nAI_Difficulty = 10)
{
    ...
    // ID - EDIT
    // Turn off Detect Modes the fight is on.
    if (GetDetectMode(OBJECT_SELF))
        {
        SetActionMode(OBJECT_SELF,ACTION_MODE_DETECT,DETECT_MODE_PASSIVE);
        SetSpawnInCondition(NW_FLAG_SEARCH,FALSE);
        }
    // ----------------------------------------------------------------------------------------
